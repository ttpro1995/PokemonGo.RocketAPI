...
namespace PokemonGo.RocketAPI.Api.PokemonModels
{
    public class Pokemon : PokemonDetails
    {
        ...
        public async Task<ReleasePokemonResponse.Types.Result> TransferPokemon()
        {
		    ...
		    if (response.Result == ReleasePokemonResponse.Types.Result.Success) {
			    _client.Inventories.PokeBank.RemovePokemon(this);
		    }
            */

            _client.Inventories.PokeBank.RemovePokemon(this);

            await _client.Inventories.UpdateInventories();
	 ...
	 */
        ...
        {
            return GetCandy() >= GetCandyCostsForPowerup() && _client.PlayerProfile
        ...
        public async Task<EvolutionResult> Evolve()
        {
            ...
            EvolutionResult result = new EvolutionResult(_client, response);
        ...
        public async Task<UseItemPotionResponse.Types.Result> Heal()
        {
		    ...
		    if (_client.Inventories.ItemBag.GetItem(ItemId.ItemPotion).Count > 0)
			    return await UsePotion(ItemId.ItemPotion);

		    if (_client.Inventories.ItemBag.GetItem(ItemId.ItemSuperPotion).Count > 0)
                return await UsePotion(ItemId.ItemSuperPotion);

		    if (_client.Inventories.ItemBag.GetItem(ItemId.ItemHyperPotion).Count > 0)
                return await UsePotion(ItemId.ItemHyperPotion);

		    if (_client.Inventories.ItemBag.GetItem(ItemId.ItemMaxPotion).Count > 0)
        ...
        {
            Item potion = _client.Inventories.ItemBag.GetItem(itemId);
        ...
        public async Task<UseItemReviveResponse.Types.Result> Revive()
        {
		    ...
		    if (_client.Inventories.ItemBag.GetItem(ItemId.ItemRevive).Count > 0)
			    return await UseRevive(ItemId.ItemRevive);

		    if (_client.Inventories.ItemBag.GetItem(ItemId.ItemMaxRevive).Count > 0)
        ...
        {
            Item item = _client.Inventories.ItemBag.GetItem(itemId);
