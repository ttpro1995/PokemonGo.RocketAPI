    ...
    public class Inventories : BaseRpc
    {
	    ...
	    public void UpdateInventories(GetInventoryResponse response)
        {
            foreach (InventoryItem inventoryItem in response.InventoryDelta.InventoryItems)
            {
                ...
                {
                    Hatchery.AddEgg(new EggPokemon(_client, itemData.PokemonData));
                }

                // PokeBank
                if (itemData.PokemonData != null && itemData.PokemonData.PokemonId != PokemonId.Missingno)
                {
                    PokeBank.AddPokemon(new Pokemon(_client, inventoryItem.InventoryItemData.PokemonData));
                }
                ...
                {
                    _client.PlayerProfile.Stats = new Stats(itemData.PlayerStats);
                }
                ...
                if (itemData.EggIncubators != null)
                {
                    ...
                    {
                        Incubators.Add(new EggIncubator(_client, incubator));
                    }
                }

                LastInventoryUpdate = Client.GetCurrentTimeMillis();
            }
        }
    }
}
